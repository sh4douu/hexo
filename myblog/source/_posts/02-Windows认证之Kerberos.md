...
Kerberos起源于希腊神话，是一支守护着冥界长着3个头颅的神犬，在Kerberos认证中，Kerberos的3个头颅也代表认证过程中涉及的三方：Client、Server、KDC。
...
> 生成`K(c,s)`的两份Copy，一份加密用于给Client，一份加密为TGT用于给Server（但先发给Client，让其发给Server）。
